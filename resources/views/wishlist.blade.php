                ...
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="row">
                                <div class="col-6">
                                    {{-- Form to move wishlist items to cart --}}
                                ...
                                <div class="col-6">
                                    {{-- Form to remove an item from wishlist --}}
                                    <form method="POST" action="{{ route('wishlist.item.remove',$item->rowId) }}" id="remove-item{{ $item->id}}">
                                        @csrf
                                        @method('DELETE')
                                        <a href="javascript:void(0)" class="remove-item" onclick="document.getElementById('remove-item{{ $item->id}}').submit();"> 
        ...
        @else
            <div class="row">
                <div class="col md-12">
                    ...
                    <a href="{{ route('shop.index') }}" class="btn btn-info">Add to wishlist now</a>
                </div>
            </div>
        @endif
      </div>
